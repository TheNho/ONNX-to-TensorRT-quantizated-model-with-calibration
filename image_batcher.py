...
def preprocess_image(image_path, format, im_width, im_height, dtype, mode='resize_and_pad', normalize=True):
        """
        ...
        * EfficientDet: Resizes and pads the image to fit the input size.
        ...
        :mode: resize_and_center_drop/resize_and_pad
        """
        def resize_pad(image, pad_color=(0, 0, 0)):
            ...
            pad = Image.new("RGB", (im_width, im_height))
            pad.paste(pad_color, [0, 0, im_width, im_height])
            pad.paste(image)
            return pad, scale
        def resize_centerdrop(image:Image):
            width, height = image.size
            width_scale = width / im_width # 640,480 -> 240,240
            height_scale = height / im_height
            scale = 1.0 / min(width_scale, height_scale)
            image = image.resize((round(width * scale+1), round(height * scale+1)), resample=Image.BILINEAR)
        ...
        image, scale = resize_centerdrop(image) if mode=="resize_and_center_drop" else resize_pad(image, (124, 116, 104))
...
class ImageBatcher:
    ...
    def __init__(self, input, shape, dtype, max_num_images=None, exact_batches=False, preprocessor=preprocess_image):
        ...
        :preprocessor: Set the preprocessor function to use, you must chage this function for other preprocessing images
        """
    ...
    def get_batch(self):
        ...
        for i, batch_images in enumerate(self.batches):
            ...
            for i, image in enumerate(batch_images):
                self.image_index += 1
                batch_data[i], batch_scales[i] = self.preprocessor(image, 
                                                                   self.format, 
                                                                   self.width, 
                                                                   self.height, 
                                                                   self.dtype)
